...
def udm_search(query: str = "", udm_query: str = "", hours_back: int = 24, max_events: int = 100, start_time: str = "", end_time: str = "") -> str:
    """[SECOPS CHRONICLE] Direct UDM queries."""
    try:
        ...
        start_time_dt = end_time_dt - timedelta(hours=hours_back)
        result = chronicle.search_udm(query=final_query, start_time=start_time_dt, end_time=end_time_dt, max_events=max_events)
        events = result.get("events", []) if isinstance(result, dict) else result
        return json.dumps({"events": events[:max_events], "count": len(events), "query": final_query})
    except Exception as e:
